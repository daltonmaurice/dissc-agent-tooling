---
title: "Agents tooling under the hood"
format:
  revealjs:
    theme: default
    incremental: false
...
## 📈 Towards Agentic chatbots {.smaller}
...
## 👨‍💻 What does using an Agentic Chatbot look like? {.smaller}
...
-   Goal is to explore setting up a remove MCP server and connecting to a chatbot on pophive website
...
## Caution everything the MCP server returns consumes tokens
...
## Lastly let the host now {.smaller}
...
-   RAG is a way to inject context to a task for an LLM to **add context** to a task based on the prompt given

-   MCP is more flexible and can add context in a dynamic way
